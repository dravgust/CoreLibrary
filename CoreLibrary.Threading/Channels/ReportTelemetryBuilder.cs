...
{
    public class ReportTelemetryBuilder<T> where T : ChannelHandlerTelemetrySnapshot
    {
        private readonly List<T> _snapshots;

        public ReportTelemetryBuilder(List<T> channelsSnapshots)
        {
            Guard.NotNull(channelsSnapshots, nameof(channelsSnapshots));
        ...
        public QueueHandlerTelemetryReport Build()
        {
            ...
            if (_snapshots.Count > 0)
            {
                ...
                var queueBuilder = new ChannelTelemetryBuilder<T>(_snapshots, _snapshots.Sum(i => i.Length));
